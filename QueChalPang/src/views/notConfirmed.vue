...
const centerInfo = reactive({ centerNo: 0, centerName: '', centerTel: '', centerAddress: '' })
...
onBeforeMount(() => {
  ...
  centerInfo.centerNo = isLogIn.value.info.center_no
  getCenterInfo()
...
const getCenterInfo = async () => {
  let result = await axios.get(`/api/center/${centerInfo.centerNo}`)
  console.log(result)
  const { center_name, center_tel, center_address } = result.data
  centerInfo.centerName = center_name
  centerInfo.centerTel = center_tel
  centerInfo.centerAddress = center_address
}
</script>
<template>
  ...
  <main class="main-content mt-0">
    <div
      class="page-header min-vh-100 border-radius-lg d-flex align-items-center justify-content-center"
      style="
      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-lg-8 col-md-10 text-center position-relative z-index-1">
            <div class="mb-4">
              <img
                src="https://cdn-icons-png.flaticon.com/512/6329/6329090.png"
                alt="모래시계"
                style="width: 120px"
              />
            </div>
            ...
            <p class="text-black lead mb-4 opacity-9">
              ...
              <strong
                >{{ centerInfo.centerName
                  ...
                  centerInfo.centerTel == '연락처 없음' ||
                  centerInfo.centerTel == '' ||
                  centerInfo.centerTel == undefined
                    ? '담당센터측'
                    : `( 연락처 : ${centerInfo.centerTel} )`
                }}으로 문의</strong
              >
              부탁드립니다.
              <br />
              {{
                centerInfo.centerAddress == undefined ? '' : ` 주소 : ${centerInfo.centerAddress}`
              }}
            </p>

            <div class="mt-5">
              <button
                class="btn btn-outline-white btn-lg px-5 border-radius-pill"
                @click="clickBtn()"
              >
                돌아가기
              </button>
